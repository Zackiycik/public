...
module.exports = {
	...
	run: async (client, message, args, embed) => {
        ...
        if (message.channel.id =! "Channel İd") return message.channel.send("Bu komut sadece <#BOT COMMANDS İD> kanalında kullanılır"); 
    ...
    ${currentRank ? `**───────────────** 
    **<a:tik:851898390425370634> Yetki Durumu:** 
    ...
    **───────────────**
    **<a:tik:851898390425370634> Ses Bilgileri:**
  <:tik:859044081812373534> Toplam: \`${moment.duration(voiceData ? voiceData.topStat : 0).format("H [saat], m [dakika] s [saniye]")}\`
  <:tik:859044081812373534> Public Odalar: \`${await category(conf.publicParents)}\`
  <:tik:859044081812373534> Kayıt Odaları: \`${await category(conf.registerParents)}\`
  <:tik:859044081812373534> Sorun Çözme & Terapi: \`${await category(conf.solvingParents)}\`
  <:tik:859044081812373534> Private Odalar: \`${await category(conf.privateParents)}\`
  <:tik:859044081812373534> Game Odalar: \`${await category(conf.aloneParents)}\`
  <:tik:859044081812373534> Oyun & Eğlence Odaları: \`${await category(conf.funParents)}\`
  <:tik:859044081812373534> Diğer: \`${await category(filteredParents.map(x => x.id))}\`
    **───────────────**
    **<a:tik:851898390425370634> Sesli Kanal Bilgileri: (\`Toplam ${voiceLength} kanal\`)**
    ${voiceTop}
    **───────────────**
    **<a:tik:851898390425370634> Mesaj Bilgileri: (\`Toplam ${messageData ? messageData.topStat : 0} mesaj\`)**
    ${messageTop}
    **───────────────**
    ${coinStatus} 
    `)
    embed.addField("<a:tik:851898390425370634> Ses Verileri:", `
     <:tik:859044081812373534> Haftalık Ses: \`${voiceWeekly}\`
     <:tik:859044081812373534> Günlük Ses: \`${voiceDaily}\`
    `, true);
    embed.addField("<a:tik:851898390425370634> Mesaj Verileri:", `
    <:tik:859044081812373534> Haftalık Mesaj: \`${Number(messageWeekly).toLocaleString()} mesaj\`
    <:tik:859044081812373534> Günlük Mesaj: \`${Number(messageDaily).toLocaleString()} mesaj\`
    `, true);
