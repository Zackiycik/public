...
exports.run = async (client, message, args) => {
  ...
  if (!args[0]) {
    ...
    const rhode = await db.fetch(
      `afkSebep_${message.author.id}_${message.guild.id}`
    );

   message.channel.send(new MessageEmbed().setColor('RANDOM').setDescription(`${kullanıcı} \n\n \`\`\`Başarıyla Afk Oldunuz Durumunuzu Afk Olarak Ayarladım\`\`\` \n\nAfk Olmanızın Sebebi: **${rhode}**`));
