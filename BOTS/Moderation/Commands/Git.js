...
if (!kullanıcı) return message.channel.send(new MessageEmbed().setDescription(`Odasına gitmek istedğiniz kullanıcıyı belirtmeniz gerekir @Vallens/ID gibi`).setAuthor(message.member.displayName, message.author.avatarURL({ dynamic: true })).setColor('RANDOM').setTimestamp()).then(x => x.delete({timeout: 5000}));
...
await mesaj.react((Other.General.yes))
await mesaj.react((Other.General.no))
