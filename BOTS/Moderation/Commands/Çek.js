...
exports.run = async (client, message, args) => {
  ...
  return message.channel.send(new MessageEmbed().setDescription(`İlk önce ses kanallarından birine girmelisin. Girdikten sonra tekrar denersen olacaktır.`).setFooter(client.user.username,client.user.avatarURL({dynamic:true})).setAuthor(message.author.tag, message.member.user.avatarURL({dynamic: true})).setColor(`RANDOM`)).then(msg => msg.delete({timeout: 5000}), message.react(Other.General.no))  } 
  let kullanıcı = message.mentions.members.first() || message.guild.members.cache.get(args[0]) 
  if (!kullanıcı) return message.channel.send(vegasembed.setDescription(`Lütfen bir üyeyi eyesetle ve tekrar dene!`)).then(msg => msg.delete({timeout: 5000}), message.react(Other.General.yes))
  if (kullanıcı.user.bot) return message.channel.send(vegasembed.setDescription(`Botlara herhangi bir işlem uygulayamazsın.`)).then(msg => msg.delete({timeout: 5000}), message.react(Other.General.yes))
  if(kullanıcı.id == (`${message.author.id}`)) return message.channel.send(vegasembed.setDescription(`Kendine herhangi bir işlem uygulayamazsın.`)).then(msg => msg.delete({timeout: 5000}), message.react(Other.General.yes))
  if (!kullanıcı.voice.channel) return message.channel.send(vegasembed.setDescription(`Belirttiğin Kişi herhangi bir ses kanalında değil.`)).then(msg => msg.delete({timeout: 5000}), message.react(Other.General.yes))
  ...
  return [Other.General.yes, Other.General.no].includes(reaction.emoji.id) && user.id === kullanıcı.id;
  };
  ...
  .then(m => m.react(Other.General.yes)
  .then(a => m.react(Other.General.no))
  .then(s =>
  m 
  ...
  if (reaction.emoji.id === Other.General.yes) {
  ...
  if (reaction.emoji.id === Other.General.no) {
  message.react(Other.General.no)
